 ...
 * comments
 */
Ext.namespace('com.ms.basic.RolePanel');
com.ms.basic.RolePanel=Ext.extend(Ext.Panel, {
	initComponent: function() {
		var roleUserWin = new com.ms.basic.RoleUserWin({
			closeAction:'hide',
			modal: true,
			width: 700,
			height: 350,
			...
			items: [
				...
				new Ext.form.ComboBox({
					...
					store: new Ext.data.SimpleStore({
						fields: ['value', 'text'],
						data: [['person', '个人权限'],['team', '组内权限'],['all', '超级权限']]
					})
				}),
			...
			buttons: [
				{
					...
					handler: function() {
						...
						editForm.form.doAction('submit', {
							url: '../basic/RoleController/save.sdo',
							method: 'post',
							waitTitle:'请等待',
							waitMsg: '正在提交...',
							params: '',
		...
		var treeLoader = new Ext.tree.TreeLoader({dataUrl: '../basic/RoleAuthorityController/getRoleResource.sdo'})
		...
		var roleResourceWin = new Ext.Window({
			...
			buttons: [{
	            ...
	            handler: function(){
	                ...
	                Ext.Ajax.request({
						method: 'post',
						url: '../basic/RoleAuthorityController/saveRoleAuthority.sdo',
		...
		var cm = new Ext.grid.ColumnModel([
			...
			{header:'数据权限', dataIndex:'roleAuth', sortable:true,
				renderer:function(v) {
					if(v=='person')
						return '个人权限';
					else if(v == 'all') 
		...
		var ds = new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({url: '../basic/RoleController/search.sdo'}),
			remoteSort: true,
			paramNames: {
				...
				buttons: [
					...
					},
					{
						...
						handler: function() {
							var rs = grid.getSelectionModel().getSelected();
							roleUserWin.show();
							roleUserWin.loadData(rs.data.id);
						}
					}
				]
			}),
		...
		function showInfo(id) {
			editForm.load({
				url: '../basic/RoleController/getDetailInfo.sdo',
				params: {id: id},
				success:function(form,action){
				}
			});
        ...
        //调用父类构造函数（必须）  
        com.ms.basic.RolePanel.superclass.initComponent.apply(this, arguments);
	},
