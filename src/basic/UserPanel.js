 ...
 * comments
 */
Ext.namespace('com.ms.basic.UserPanel');
com.ms.basic.UserPanel=Ext.extend(Ext.Panel, {
	initComponent: function() {
			...
			buttons: [
				{
					...
					handler: function() {
						...
						editForm.form.doAction('submit', {
							url: '../basic/UserController/save.sdo',
							method: 'post',
							waitTitle:'请等待',
							waitMsg: '正在提交...',
							params: '',
		...
		var rolesDS = new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({url: '../basic/UserRoleController/getUserUnhaveRoles.sdo'}),
			remoteSort: true,
			paramNames: {
		...
		var userRolesDS = new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({url: '../basic/UserRoleController/getUserRoles.sdo'}),
			remoteSort: true,
			paramNames: {
				start : 'extLimit.start', 
			    limit : 'extLimit.limit', 
			...
			buttons: [
				{
		            ...
		            handler: function(){
		            	Ext.MessageBox.confirm('提示', '确定保存 ?', function(btn) {
							...
							Ext.Ajax.request({
								method: 'post',
								url: '../basic/UserRoleController/save.sdo',
		...
		var ds = new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({url: '../basic/UserController/search.sdo'}),
			remoteSort: true,
			paramNames: {
				start : 'extLimit.start', 
			    limit : 'extLimit.limit', 
        ...
        //调用父类构造函数（必须）  
		com.ms.basic.UserPanel.superclass.initComponent.apply(this, arguments);
	},
